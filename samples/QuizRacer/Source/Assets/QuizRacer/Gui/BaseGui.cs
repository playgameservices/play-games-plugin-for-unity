// <copyright file="BaseGui.cs" company="Google Inc.">
...
public class BaseGui : MonoBehaviour
{
    ...
    public class WidgetConfig
    {
        ...
        public WidgetConfig(float centerX, float centerY,
                            float width, float height, float fontSize, string text)
            : this(WidgetAnchor.Center, centerX,
    ...
    // convert our device-independent logical coordinate system
    ...
    {
        return new Rect(ScreenX(centerX - width / 2f),
            ScreenY(centerY - height / 2f),
            ScreenDim(width),
            ScreenDim(height));
    }
    ...
    protected void GuiProgressBar(WidgetConfig cfg,
        Texture fgTexture,
        Texture bgTexture,
        float percent)
    {
    ...
    protected bool GuiButton(WidgetConfig config, string overrideText)
    {
        if (!mStandBy)
        {
            ...
            return GUI.Button(GetWidgetRect(config),
                overrideText == null ? config.Text : overrideText);
        }
        else
        {
