...
public class PlayGui : BaseGui
{
    ...
    private string ExplainWhyICantPlay()
    {
        switch (mMatch.Status)
        {
            ...
            case TurnBasedMatch.MatchStatus.Complete:
                return mMatchData.Winner == mMyMark ? "Match finished. YOU WIN!" : "Match finished. YOU LOST!";
    ...
    private void SetupObjects(bool canPlay)
    {
        ...
        {
            Spawn(GetBlocksRoot(), desc.mark == MatchData.MarkX ? XBlockPrefabName : OBlockPrefabName, desc.position,
                desc.rotation);
        }
        ...
        if (canPlay)
        {
            ...
            GameObject o = Spawn(GetBlocksRoot(), mMyMark == MatchData.MarkX ? XBlockPrefabName : OBlockPrefabName);
    ...
    void FinishMatch()
    {
        ...
        PlayGamesPlatform.Instance.TurnBased.Finish(mMatch, mMatchData.ToBytes(),
            ...
            {
                EndStandBy();
                mFinalMessage = success ? (winnerIsMe ? "YOU WON!" : "YOU LOST!") : "ERROR finishing match.";
            });
    }
    ...
    void UpdateLargeMarks(char mark, string markName)
    {
        ...
        for (x = 0; x < MatchData.BoardSize; x++)
        {
            for (y = 0; y < MatchData.BoardSize; y++)
            {
                ...
                GameObject.Find(objName).GetComponent<Renderer>().enabled =
                    (mMatchData != null) ? (mMatchData.GetMark(x, y) == mark) : false;
            }
        }
    }
