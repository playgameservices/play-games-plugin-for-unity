...
public class NativeClient : IPlayGamesClient {
	...
	private volatile GooglePlayGames.BasicApi.Multiplayer.Player mUser = null;
	...
	private static Action<T> AsOnGameThreadCallback <T> (Action<T> callback) {
		...
		private static void InvokeCallbackOnGameThread <T> (Action<T> callback, T success) {
		if (callback == null) {
			return;
		}
	...
	public void CurrentScoreValue (string leaderboardId, Action<ulong> callback){
		...
		GameServices().LeaderboardManager ().CurrentScoreValue (leaderboardId, callback);
