...
public class PlayGamesPlatform : ISocialPlatform {
	...
	public void GetScore(string board, Action<ulong> callback) {
		if (!IsAuthenticated()) {
			Logger.e("GetScore can only be called after authentication.");
			if (callback != null) {
				callback.Invoke(System.UInt64.MaxValue);
			}
			return;
		}
		
		Logger.d("GetScore: board=" + board);
		string lbId = MapId(board);
		mClient.CurrentScoreValue(lbId, callback);
	}
	...
	public void ShowLeaderboardsUI() {
		if (!IsAuthenticated()) {
			Logger.e("ShowALeaderboardsUI can only be called after authentication.");
			return;
		}
		
		Logger.d("ShowLeaderboardsUI");
		mClient.ShowLeaderboardsUI ();
	}
